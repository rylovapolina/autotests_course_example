...
class PersonInfo:
    ...
    def short_name(self):
        """ Функция возвращает отформатированное имя """
        fio_split = self.fio.split()
        fio_split = f'{fio_split[1]} {fio_split[0][0]}.'
        return fio_split

    def path_deps(self):
        """ Функция возвращает отформатированный путь работы сотрудника"""
        list_work = self.work
        result = list_work[0]
        if len(list_work) > 1:
            for j in list_work[1:]:
                result += f' --> {j}'
        print(result)
        return result

    def new_salary(self):
        """ Функция считает новую зарплату"""
        new_str = ''
        max_value = 0
        ...
        dict_max = {}
        for i in new_str:
            dict_max[i] = new_str.count(i)
            max_value = sum(sorted(dict_max.values())[-3:])
        return 1337 * self.age * max_value
