...
class Segment:
    ...
    def length(self):
        """ Функция считает длину отрезка и округляет до двух знаков после запятой"""
        len_vector = round(math.sqrt((self.point_1[0]-self.point_2[0]) ** 2 + (self.point_1[1]-self.point_2[1]) ** 2), 2)
        return len_vector

    def x_axis_intersection(self):
        """ Функция проверяет, пересекает ли отрезок ось абцисс """
        sign_1 = self.point_1[0] > 0
        sign_2 = self.point_2[0] > 0
        return sign_2 != sign_1

    def y_axis_intersection(self):
        """ Функция проверяет, пересекает ли отрезок ось аординат """
        sign_1 = self.point_1[1] > 0
        sign_2 = self.point_2[1] > 0
        return sign_2 != sign_1
